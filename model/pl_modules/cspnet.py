...
from tools.data_utils import lattice_params_to_matrix_torch, get_pbc_distances, radius_graph_pbc, frac_to_cart_coords, repeat_blocks
...
class CSPNet_outputall(nn.Module):
    ...
    def __init__(
        ...
        super(CSPNet_outputall, self).__init__()
        ...
        if self.use_time:
            ...
            self.atom_latent_emb = nn.Linear(hidden_dim , hidden_dim)
        if act_fn == 'silu':
            self.act_fn = nn.SiLU()
        if dis_emb == 'sin':
            self.dis_emb = SinusoidsEmbedding(n_frequencies=num_freqs)
        ...
        if self.pred_scalar:
            self.scalar_out = nn.Linear(hidden_dim, 1)
        self.pred_other_scalar = pred_other_scalar
        if pred_other_scalar:
            self.other_scalar_out = nn.Sequential(nn.Linear(hidden_dim, hidden_dim),nn.SiLU(),nn.Linear(hidden_dim, 1))
    ...
    def forward(self, t, atom_types, frac_coords, lattices, num_atoms, node2graph):
        ...
        type_out = self.type_out(node_features)
        if self.pred_other_scalar:
            return lattice_out, coord_out, type_out, self.scalar_out(graph_features),self.other_scalar_out(graph_features)
        else:
            return lattice_out, coord_out, type_out, self.scalar_out(graph_features)
