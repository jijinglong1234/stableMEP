...
def kappa_func():
    ...
    # for normalization
    kappa_matrix = torch.load(Path(model.__file__).parent / 'normalization' / 'kappa_matrix.pth')
    kappa_norm = torch.load(Path(model.__file__).parent / 'normalization' / 'kappa_norm.pth')
...
class SigmaScheduler(nn.Module):
    ...
    def __init__(
        ...
        self.register_buffer('kappa_matrix', torch.cat([torch.ones([max_atoms+1, 1]), kappa_matrix_], dim=1))
        self.register_buffer('kappa_norm', torch.cat([torch.ones([max_atoms+1, 1]), kappa_norm_], dim=1))
