...
from model.model_utils import RBFExpansion, BaseSettings
...
class Matformer_knowledge(nn.Module):
    """att pyg implementation."""
    ...
    def forward(self, data, edge_attr=None) -> torch.Tensor:
        if len(data)>3:
            node_pos_list = data[-1]
            data = data[0:-1]
        data, ldata, lattice = data
        # initial node features: atom feature network...
        ...
        if edge_attr is not None:
            node_features, force_logits = self.att_layers[4](node_features, data.edge_index, edge_features, diff = edge_attr)
        else:
            node_features = self.att_layers[4](node_features, data.edge_index, edge_features)
        ...
        if edge_attr is not None:
            return torch.squeeze(out), force_logits
        else:
            return torch.squeeze(out)

class Matformer_denoising(nn.Module):
    """att pyg implementation."""
    ...
    def forward(self, data, edge_attr=None) -> torch.Tensor:
        ...
        node_features = self.att_layers[2](node_features, data.edge_index, edge_features)
        node_features = self.att_layers[3](node_features, data.edge_index, edge_features)
        node_features, force_logits = self.att_layers[4](node_features, data.edge_index, edge_features,
                                                         diff=data.edge_attr)
        # if edge_attr is not None:
        #     node_features, force_logits = self.att_layers[4](node_features, data.edge_index, edge_features, diff = data.edge_attr)
        # else:
        #     node_features = self.att_layers[4](node_features, data.edge_index, edge_features)
