...
if __name__=='__main__':
    ...
    for n in range(2, max_atoms+1):
        ...
        torch.save(kappa_matrix, '../model/normalization/kappa_matrix_'+str(n)+'.pth')
        torch.save(kappa_norm, '../model/normalization/kappa_norm_'+str(n)+'.pth')
        a = 1
    
    sigmas = torch.FloatTensor(np.exp(np.linspace(np.log(0.005), np.log(0.5), 1000))).to('cuda')
    sample_norm_ = sample_norm(sigmas, num_atoms=max_atoms)
    torch.save(sample_norm_, '../model/normalization/sample_norm_'+str(max_atoms)+'.pth')
